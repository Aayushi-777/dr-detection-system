...
def create_patient(db: Session, patient_id: str, name: str='Unknown', age: int=0, gender: str='Unknown'):
...
def save_prediction(db: Session, patient_id: str, image_name: str,
    ...
    pred = Prediction(
        ...
        prob_no_dr      = round(float(probs[0]), 4) if len(probs) > 0 else 0.0,
        prob_mild       = round(float(probs[1]), 4) if len(probs) > 1 else 0.0,
        prob_moderate   = round(float(probs[2]), 4) if len(probs) > 2 else 0.0,
        prob_severe     = round(float(probs[3]), 4) if len(probs) > 3 else 0.0,
        prob_pdr        = round(float(probs[4]), 4) if len(probs) > 4 else 0.0,
...
def save_report(db: Session, prediction_id: int, pdf_path: str, doctor_notes: str=''):
