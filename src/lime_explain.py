...
def build_predict_fn(model, device, img_size=300):
    """
...
def generate_lime_explanation(model, image_np, pred_class, device='cuda',
                              img_size=300, num_samples=1000, num_features=8,
                              positive_only=True):
    """
    ...
    temp_img, mask=explanation.get_image_and_mask(
        pred_class, positive_only=positive_only,
...
def plot_lime_analysis(image_np, overlay, top_segments, pred_class, probs, save_path=None):
    """
    ...
    axes[1].set_title(f'LIME — top regions supporting\n"{DR_CLASSES[pred_class]}"', fontweight='bold')
    axes[1].axis('off')
    ...
    plt.suptitle(f'LIME Analysis  |  Confidence: {probs[pred_class]*100:.1f}%', fontsize=13, fontweight='bold')
    plt.tight_layout()
