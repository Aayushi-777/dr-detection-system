...
def generate_pdf_report(patient_id, image_name, figure_path,
                        class_idx, probs, active_regions, output_path=None):
    import os
    import datetime
    ...
    if output_path is None:
        filename = f"report_{patient_id}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
    ...
    try:
        ...
        story.append(Paragraph("Diabetic Retinopathy Screening Report", styles['Title']))
        ...
        table_data = [
            ...
            ['Risk Level', risk],
            ['Regions of Concern', ', '.join(active_regions) if active_regions else 'N/A'],]
